...
namespace Avalonia.Markup.Xaml.MarkupExtensions
{
    public class ThemeResourceExtension : IBinding
    {
            ...
            bool enableDataValidation)
        {
            ...
            if (control != null)
            {
                var source = control.GetThemeResourceObservable(ResourceKey, GetConverter(targetProperty));
                return InstancedBinding.OneWay(source);
            }
            else if (_anchor is IResourceProvider resourceProvider)
            {
                var source = resourceProvider.GetThemeResourceObservable(ResourceKey, GetConverter(targetProperty));
                return InstancedBinding.OneWay(source);
            }
